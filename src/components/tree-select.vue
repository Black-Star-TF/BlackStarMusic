...
<!-- :multiple="multiple" -->
  ...
  >
    <el-option style="display: none"  v-for="item in selectOptions" :key="item[treeProps.value]"  :value="item[treeProps.value]" :label="item[treeProps.label]"/>
    <el-input size="small" ref="search" v-model="searchKeyword" placeholder="输入以进行搜索"></el-input>
    <el-tree 
      class="select-tree"
      :data="options"
      :props="treeProps"
      ...
      ref="tree"
    >
      <span class="custom-tree-node" :class="getCurrentClass(data)" slot-scope="{ node, data }">
  ...
  methods: {
    handleVisibleChange(visible){
      if(!visible){
        ...
        this.$nextTick(()=>{
          let value = this.multiple ? this.value : (this.value==undefined ? [] : [this.value])
          value.forEach(item=>{
    ...
    getCurrentClass(node){
      if(!this.multiple){
        ...
        return this.value.includes(node[this.treeProps.value]) ? 'currentNode' : null
      }
    },
    // 将树结构转化为数组
    ...
    handleNodeClick(nodeData){
      if(!this.multiple){
        ...
        let index = this.value.findIndex(item => item === nodeData[this.treeProps.value])
