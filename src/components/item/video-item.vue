<template>
	 <div class="video-item" :style="style">
		 <div class="video-cover" @click="playVideo(video.vid)" :style="{'backgroundImage':`url(${coverUrl}?param=500y280)`}">
				<!-- 播放量 -->
				<span class="video-playcount"><span class="iconfont icon-pause"></span> {{playCount | formatCount}}</span>
				<!-- 时长 -->
				<span class="video-duration">{{duration | formatDuration}}</span>
			</div>
			<div class="video-name" @click="playVideo(video.vid)"><span class="mv-icon" v-if="isMV">MV</span> <span>{{name}}</span></div>
			<!-- 歌手 -->
			<div class="video-artists" v-if="isMV">
				<span class="artist-item" v-for="(artist,index) in video.artists" :key="artist.userId">
					<span class="name" @click="toArtistDetail(artist.id)"><span>{{artist.name}}</span></span>
					<span class="separator" v-if="index < video.artists.length - 1">/</span>
				</span>
			</div>
			<!-- 作者 -->
			<div class="video-creator" v-else>
				by 
				<span class="creator" @click.stop="toUserDetail(video.creator.userId)">{{video.creator.nickname}}</span>
			</div>
	 </div>
</template>
