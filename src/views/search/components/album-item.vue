  ...
  <div class="album-item" @click="toAlbumDetail(album.id)">
    ...
    <div class="album-name">
      <span class="name">
        ...
        <!-- 专辑别名 -->
        <span class="album-alias" v-if="album.alias.length > 0"
          >(<span v-html="markKeywords(album.alias[0])"></span>)</span
        >
      </span>
    </div>
    <!-- 歌手 -->
    <div class="album-artist">
      <span class="artist-name" @click.stop="toArtistDetail(album.artist.id)">
        ...
        <span class="artist-alias" v-if="album.artist.alias.length > 0"
          >(<span v-html="markKeywords(album.artist.alias[0])"></span>)</span
        >
      </span>
    </div>
  </div>
