  ...
  <div class="video-item" :style="style">
    ...
    >
      ...
      <span class="video-playcount"
        ...
        {{ video.playTime | formatCount }}</span
      >
      <!-- 时长 -->
      <span class="video-duration">{{
        video.durationms | formatDuration
      }}</span>
    </div>
    <div class="video-name" @click="playVideo(video.vid)">
      <span class="mv-icon" v-if="isMV">MV</span>
      <span v-html="markKeywords(video.title)"></span>
    </div>
    <!-- 歌手 -->
    <div class="video-artists" v-if="isMV">
      <span
        class="artist-item"
        v-for="(artist, index) in video.creator"
        :key="artist.userId"
      >
        <span class="name" @click="toArtistDetail(artist.userId)"
          ><span v-html="markKeywords(artist.userName)"></span
        ></span>
    ...
    <div class="video-creator" v-else>
      ...
      <span
        class="creator"
        @click.stop="toUserDetail(video.creator[0].userId)"
        v-html="markKeywords(video.creator[0].userName)"
      ></span>
    </div>
  </div>
</template>
