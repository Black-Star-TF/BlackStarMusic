<template>
  <div class="multiple-match-container">
    ...
    <template v-if="result.artist">
      <div class="multiple-match-item artist" @click="toArtistDetail(result.artist[0].id)">
        <img :src="result.artist[0].img1v1Url" class="cover">
        <span>歌手：{{result.artist[0].name}} <span class="alias" v-if="result.artist[0].alias.length>0">({{result.artist[0].alias[0]}})</span></span>
      </div>
    </template>
    <!-- 用户 -->
    <template v-if="result.user">
      <div class="multiple-match-item user" @click="toUserDetail(result.user[0].userId)">
    ...
    <template v-if="result.album">
      <div class="multiple-match-item album" @click="toAlbumDetail(result.album[0].id)">
