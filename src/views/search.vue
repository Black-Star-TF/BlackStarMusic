  ...
  <div class="view-search">
    ...
    <div class="container">
      ...
      <template v-if="results&&type==100">
        <singer-item v-for="singer in results" :key="singer.id" :singer="singer" :keyword="keyword"></singer-item>
      </template>
      <!-- 专辑 -->
      <template v-if="results&&type==10">
        <album-item v-for="album in results" :key="album.id" :album="album" :keyword="keyword"></album-item>
      </template>
      <!-- 视频 -->
      <template v-if="results&&type==1014">
        <div class="video-container">
          <video-item v-for="(video,index) in results" :key="video.vid" :num="num" :keyword="keyword" :video="video" :index="index"></video-item>
        </div>
      </template>
      <!-- 歌单 -->
      <template v-if="results&&type==1000">
        <playlist-item
      v-for="playlist in results" :key="playlist.id" :playlist="playlist" :keyword="keyword"></playlist-item>
      </template>
      <!-- 主播电台 -->
      <template v-if="results&&type==1009">
        <radio-item v-for="radio in results" :key="radio.id" :radio="radio" :keyword="keyword"></radio-item>
      </template>
      <!-- 用户 -->
      <template v-if="results&&type==1002">
        <user-item v-for="user in results" :key="user.userId" :user="user" :keyword="keyword"></user-item>
      </template>
...
import SingerItem from '@/components/search/singer-item'
import AlbumItem from '@/components/search/album-item'
import PlaylistItem from '@/components/search/playlist-item'
