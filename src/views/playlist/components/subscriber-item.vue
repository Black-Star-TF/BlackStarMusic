<template>
  <div :style="style" class="subscriber-item">
    <div
      ...
      @click="toUserDetail(subscriber.userId)"
    ></div>
    <div class="user-info">
      <div class="user-name">
        <span class="name" @click="toUserDetail(subscriber.userId)">{{
          subscriber.nickname
        }}</span>
        <span
          class="gender gender-nan iconfont icon-228yonghu_xingbienan"
          v-if="subscriber.gender == 1"
        ></span>
        <span
          class="gender gender-nv iconfont icon-girl"
          v-if="subscriber.gender == 2"
        ></span>
      </div>
      <div class="user-signature" v-if="subscriber.signature != ''">
