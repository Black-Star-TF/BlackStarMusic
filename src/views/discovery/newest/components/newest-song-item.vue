<template>
  <div class="newest-song2-item">
    ...
    <div class="name">
      <div class="song-name">
        <span>{{ song.name }}</span>
        <span class="alia" v-if="song.alias.length > 0"
          >({{ song.alias[0] }})</span
        >
      </div>
      ...
      <span
        ...
        v-if="song.mvid != 0"
        @click="playVideo(song.mvid)"
      ></span>
    </div>
    <div class="artists">
      <span v-for="(artist, index) in song.album.artists" :key="artist.id">
        <span class="artist-name" @click.stop="toArtistDetail(artist.id)">{{
          artist.name
        }}</span>
        <span class="separator" v-if="index < song.album.artists.length - 1">
          /
        </span>
      </span>
    </div>
    <div class="album">
      <span
        ...
        @click.stop="toAlbumDetail(song.album.id)"
