<template>
  <div class="video-item" :style="style">
    <div
      class="video-cover"
      @click="playVideo(video.vid)"
      :style="{ backgroundImage: `url(${coverUrl}?param=500y280)` }"
    >
      <!-- 播放量 -->
      <span class="video-playcount"
        ><span class="iconfont icon-pause"></span>
        {{ playCount | formatCount }}</span
      >
      <!-- 时长 -->
      <span class="video-duration">{{ duration | formatDuration }}</span>
    </div>
    <div class="video-name" @click="playVideo(video.vid)">
      <span class="mv-icon" v-if="isMV">MV</span> <span>{{ name }}</span>
    </div>
    <!-- 歌手 -->
    <div class="video-artists" v-if="isMV">
      <span
        ...
        :key="artist.userId"
      >
        <span class="name" @click="toArtistDetail(artist.id)"
          ><span>{{ artist.name }}</span></span
        >
    ...
    <div class="video-creator" v-else>
      by
      <span class="creator" @click.stop="toUserDetail(video.creator.userId)">{{
