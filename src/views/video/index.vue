<template>
  <div class="mv-detail-page">
    <div class="main">
      <div class="title">
        <span><i class="iconfont icon-changyong_fanhui"></i>视频详情</span>
      </div>
      <video :src="videoUrl" width="100%" class="video" controls autoplay></video>
      <!-- 歌手 -->
      <div v-if="videoDetail">
        <div class="artist">
          <span class="artist-cover" @click="toArtistDetail(videoDetail.creator.userId)" :style="{'backgroundImage':`url(${videoDetail.creator.avatarUrl})`}"></span>
          <span class="artist-name" @click="toArtistDetail(videoDetail.creator.userId)">{{videoDetail.creator.nickname}}</span>
        </div>
        <div class="name">
          {{videoDetail.title}}
          <span v-if="videoDetail.description" @click="changeDesc" :class="['iconfont',icon]"></span>
        </div>
        <div class="info">
        ...
        <div class="operation">
          <span class="operation-item"><span class="iconfont icon-dianzan"></span> 赞({{videoDetail.praisedCount}})</span>
          <span class="operation-item"><span class="iconfont icon-shoucang"></span> 收藏({{videoDetail.subscribeCount}})</span>
          <span class="operation-item"><span class="iconfont icon-fenxiang1"></span> 分享({{videoDetail.shareCount}})</span>
