...
export default {
  ...
  async getUserPlaylist({ state, commit }) {
    ...
    playlist.forEach(item => {
      item.subscribed
        ? playlistsSubscribedByMe.push(item)
        : playlistsCreatedByMe.push(item);
    });
    commit("updateData", {
      key: "playlistsCreatedByMe",
      value: playlistsCreatedByMe,
    });
    commit("updateData", {
      key: "playlistsSubscribedByMe",
      value: playlistsSubscribedByMe,
    });
  },
  async getLikedSongList({ state, commit }) {
    // 获取喜欢的歌曲列表 id
    // timestamp
    let res = await getLikedSongList({
      uid: state.data.userProfile.userId,
      timestamp: new Date().getTime(),
    });
